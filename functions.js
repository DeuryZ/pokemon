...
export async function showPokemon(e,select){
    ...
    if (e.target.matches(".range")) {
        ...
        const existingStat = pokedatos.find((stat) => stat.name === pokeStatName);
            if (existingStat) {
                existingStat.value = pokeStatValue;
            } else {
                pokedatos.push({ nameStat: pokeStatName, value: pokeStatValue, namePokemon: idName });
            }
